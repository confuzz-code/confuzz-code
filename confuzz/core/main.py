...
from confuzz.utils.file_system_utils import path_is_existed
...
class MainEngine( object ):
        ...
        if self.test_mode == "Default" or self.test_mode == "MySQL" :
            ...
            for i in range( self.options_num ):
                ...
                try:
                    ...
                    self.options = self.conf_engine.conf_parser.transfer_nginx_conf( i )
    ...
    def run(self):
        if self.test_mode == "Default" or self.test_mode == "MySQL":
            ...
            for i in range( self.options_num ):
                i = str( i + 1 )
                self.options = self.conf_engine.conf_parser.transfer_nginx_conf( int( i ) )
                option = self.options[i]
                mutants = self.misconf_engine.mutate( option )
                key = option["key"]
                option["mutants"] = mutants
