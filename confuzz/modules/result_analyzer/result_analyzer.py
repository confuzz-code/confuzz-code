...
from confuzz.utils.file_system_utils import get_file_content, get_rid_of_string
from confuzz.utils.regex_expression_utils import find_pattern, remove_pattern, add_escape
from confuzz.core import config
...
class ResultAnalyzer( object ):
    ...
    def build_documents(self, directory, oracle_dict):
        ...
        if config.config.test_mode == "Default" or config.config.test_mode == "MySQL":
            for file in files:
                path = file
                title = file.split( '/' )[-1]
                if ".txt" in title:
        ...
        elif config.config.test_mode == "Httpd":
            for file in files:

                path = file
                title = file.split( '/' )[-1]
                if "console.txt" in title or "log.txt" in title:
                    if "console.txt" in title:
                        testcase_id = title.split( "_console.txt" )[0]
                    elif "log.txt" in title:
                        testcase_id = title.split( "_log.txt" )[0]
                    else:
                        testcase_id = "unknown"
                    ...
                    self.testcase_ids.append( {
                        ...
                        "path": path,
                        "title": title,
                        "log2annotate": oracle_dict["1"]["log2annotate"],
                        "log2purge": oracle_dict["1"]["log2purge"],
                    } )
        elif config.config.test_mode == "Nginx":
            for file in files:
                path = file
                title = file.split( '/' )[-1]
                if ".txt" in title:
...
def main():
    ...
    #a.set_char2cut( 0 )
    #a.build_baseline( "/Users/A/Desktop/confuzz/confuzz/core/Log", ["2018-11-02-104220.log"], [] )
